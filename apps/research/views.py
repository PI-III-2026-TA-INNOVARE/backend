...
class IsCompanyForCreate(permissions.BasePermission):
    message = 'Apenas usuarios do tipo empresa podem criar pesquisas.'
...
class ResearchCreateListView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        if user.id_type != User.UserType.EMPRESA:
            raise PermissionDenied('Apenas usuarios do tipo empresa podem criar pesquisas.')
        if not hasattr(user, 'company_profile'):
            raise PermissionDenied('Usuario empresa sem perfil de empresa vinculado.')
